...
# ── Config ──────────────────────────────────────────────────────────────────
AIWORK_ROOT="$(cd "$(dirname "$0")/.." && pwd)"

# Detect whether we're inside a factory layout (../../.. has _repos/)
FACTORY_CANDIDATE="$(cd "$AIWORK_ROOT/../../.." 2>/dev/null && pwd)"
...
if [ -n "$FACTORY_ROOT" ]; then
  for d in "$FACTORY_ROOT"/_repos/*/; do
    [ -d "$d" ] || continue
    [ "$(cd "$d" && pwd)" = "$AIWORK_ROOT" ] && continue
    SIBLING_REPOS+=("$d")
  done
fi

# ── Internal infra: all build/config/CI files that may reference source ────
# These are files WITHIN aiwork that knip can't trace but that use source files
...
INFRA_GLOBS=(
  ...
  "apps/*/scripts/*.ts"
  "apps/*/scripts/*.sh"
  # .aiwork skills/commands that may invoke source
  ".aiwork/skills/*/scripts/*.sh"
  ".aiwork/skills/*/*.sh"
)
...
collect_infra_files() {
  ...
  for glob_pattern in "${INFRA_GLOBS[@]}"; do
    ...
    local matched
    matched=$(find "$AIWORK_ROOT" -path "$AIWORK_ROOT/$glob_pattern" 2>/dev/null || true)
    if [ -n "$matched" ]; then
      files="${files}${matched}"$'\n'
    fi
  done
  ...
  local pkg_jsons
  pkg_jsons=$(find "$AIWORK_ROOT" -name package.json -not -path '*/node_modules/*' -not -path '*/.git/*')
  files="${files}${pkg_jsons}"$'\n'
  # And all tsconfig*.json files (for path aliases / includes)
  local tsconfigs
  tsconfigs=$(find "$AIWORK_ROOT" -name 'tsconfig*.json' -not -path '*/node_modules/*' -not -path '*/.git/*')
...
# Search sibling repo CI/CD and build scripts for an aiwork-relative path.
# Only checks infra files (workflows, Dockerfiles, build scripts), NOT source code,
# since no sibling repo has an npm dependency on aiwork packages.
search_sibling_ci() {
  local pattern="$1"
  local hits=""
...
format_refs() {
  ...
  while IFS= read -r ref; do
    ...
    local short="$ref"
    if [[ "$ref" == "$AIWORK_ROOT/"* ]]; then
      short="${ref#"$AIWORK_ROOT/"}"
...
for filepath in "${UNUSED_FILES[@]}"; do
  ...
  # Only search by the aiwork-relative path (precise) or unique filename.
  # Since no sibling repo has npm deps on aiwork packages, we only check
...
echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo -e "${DIM}  No imports in aiwork source, no references in CI/CD, build${RESET}"
