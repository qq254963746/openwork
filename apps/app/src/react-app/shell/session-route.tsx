...
import type { ReadGlobalEngineConfigInput } from "../../app/lib/global-engine-disabled-providers";
...
import { buildAiWorkEnvSystemContext } from "../domains/session/sync/env-context";
import {
...
export function SessionRoute() {
  ...
  const [client, setClient] = useState<AiWorkServerClient | null>(null);
  ...
  const [aiworkServerHostInfoState, setAiWorkServerHostInfoState] = useState<AiWorkServerInfo | null>(null);
  ...
  const [aiworkServerSettingsVersion, setAiWorkServerSettingsVersion] = useState(0);
  const [engineReloadVersion, setEngineReloadVersion] = useState(0);
  const reconnectAttemptedWorkspaceIdRef = useRef("");

  const aiworkServerSettings = useMemo(
    () => readAiWorkServerSettings(),
    [aiworkServerSettingsVersion],
  );
  ...
  const loadWorkspaceSessionsInBackground = useCallback(
    async (aiworkClient: AiWorkServerClient, workspaces: RouteWorkspace[]) => {
      ...
      const fetchOnce = async (workspace: RouteWorkspace, attempt: number): Promise<void> => {
        ...
        try {
          const response = await aiworkClient.listSessions(workspace.id, { limit: 200 });
  ...
  const refreshRouteState = useCallback(async () => {
    ...
    try {
      ...
      setAiWorkServerHostInfoState(hostInfo);
      ...
      const list = await aiworkClient.listWorkspaces();
      ...
      if (selectedWorkspace) {
        void loadWorkspaceSessionsInBackground(aiworkClient, [selectedWorkspace]);
      }
    } catch (error) {
      const message = describeRouteError(error);
  ...
  useEffect(() => {
    ...
    const handleSettingsChange = () => {
      setAiWorkServerSettingsVersion((value) => value + 1);
  ...
  // `window.__aiwork.snapshot()` or `window.__aiwork.slice("route")` and
  ...
  const engineBaseUrl = useMemo(() => {
    ...
    return `${mounted.replace(/\/+$|\/+$/g, "")}/engine`;
  ...
  const engineClient = useMemo(
    () =>
      engineBaseUrl && token && !selectedWorkspaceError
        ? createClient(engineBaseUrl, selectedWorkspaceRoot || undefined, {
            token,
            mode: "aiwork",
          })
        : null,
    [engineBaseUrl, selectedWorkspaceError, selectedWorkspaceRoot, token],
  );
  const canCreateTask = Boolean(
    engineClient && selectedWorkspaceId && !loading && !selectedWorkspaceError,
  );
  ...
  useEffect(() => {
    if (!engineClient || !selectedWorkspaceId || !selectedSessionId) return;
    let cancelled = false;
    const directory = selectedWorkspaceRoot || undefined;
    void (async () => {
      const snapshotStartedAt = Date.now();
      try {
        const list = unwrap(await engineClient.permission.list({ directory })) as unknown;
        if (!cancelled) {
          seedPermissionState(
            selectedWorkspaceId,
            selectedSessionId,
  ...
  }, [engineClient, selectedSessionId, selectedWorkspaceId, selectedWorkspaceRoot]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const questionApi = (engineClient as any).question;
  ...
  const respondPermission = useCallback(
    async (requestID: string, reply: "once" | "always" | "reject") => {
      ...
      try {
        unwrap(
          await engineClient.permission.reply({
            requestID,
            reply,
    ...
    [engineClient, selectedSessionId, selectedWorkspaceId, selectedWorkspaceRoot, showToast],
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const globalInput: ReadGlobalEngineConfigInput = {
      ...
      const filtered = await fetchFinallyProviderList({
        listClient: engineClient as Client,
        globalInput,
  ...
  }, [client, engineClient, selectedWorkspaceId, selectedWorkspaceRoot]);
  ...
  useEffect(() => {
    if ((!modelPickerOpen && modelsLoadId === 0) || !engineClient) return;
    let cancelled = false;
    void (async () => {
      let caps: AiWorkServerCapabilities | null = null;
      if (client) {
  ...
  const surfaceProps = useMemo(() => {
    ...
    return {
      ...
      onSendDraft: async (draft: ComposerDraft) => {
        ...
        if (draft.mode === "shell") {
          await shellInSession(engineClient, selectedSessionId, text);
        ...
        const envRuntimeKey = buildAiWorkEnvRuntimeKey({
          ...
          pid: aiworkServerHostInfoState?.pid ?? null,
          port: aiworkServerHostInfoState?.port ?? null,
        });
        const envSystemContext = await buildAiWorkEnvSystemContext(client, {
      ...
      listAgents: async () => {
        const list = unwrap(await engineClient.app.agents()) as import("@engine/sdk/v2/client").Agent[];
        return list.filter((agent: import("@engine/sdk/v2/client").Agent) => !agent.hidden && agent.mode !== "subagent" && agent.name !== "aiwork");
      },
  ...
  const handleCreateTaskInWorkspace = useCallback(async (workspaceId: string) => {
    ...
    const workspaceEngineBaseUrl = `${(buildAiWorkWorkspaceBaseUrl(baseUrl, workspace.id) ?? baseUrl).replace(/\/+$|\/+$/g, "")}/engine`;
    const workspaceClient = createClient(
      workspaceEngineBaseUrl,
  ...
  const commandPaletteControlAction = useMemo<AiWorkControlAction>(() => ({
  ...
  return (
    <>
    {engineClient && selectedWorkspaceId && engineBaseUrl && token ? (
      <ReactSessionRuntime
        workspaceId={selectedWorkspaceId}
        sessionId={selectedSessionId}
