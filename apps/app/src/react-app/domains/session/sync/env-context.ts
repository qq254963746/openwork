...
import { readAiWorkEnvPendingChanges } from "../../../../app/lib/aiwork-env-runtime";

const DEFAULT_CACHE_KEY = "__aiwork_env_default__";
const MAX_CONTEXT_CACHE_ENTRIES = 100;

const envSystemContextCache = new Map<string, string | undefined>();

export function clearAiWorkEnvSystemContextCache(): void {
...
export async function buildAiWorkEnvSystemContext(
  ...
  const readPendingChanges = options.readPendingChanges ??
    (() => readAiWorkEnvPendingChanges(options.runtimeKey));
  ...
  try {
    ...
    const context = [
      "AiWork environment variables configured:",
      keyList,
