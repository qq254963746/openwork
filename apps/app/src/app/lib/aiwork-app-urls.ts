/**
 * Normalizes hosted AiWork web app / API base URLs (formerly routed via Den client helpers).
 */

export const DEFAULT_AIWORK_APP_URL =
  (typeof import.meta !== "undefined" && typeof import.meta.env?.VITE_DEN_BASE_URL === "string"
    ? import.meta.env.VITE_DEN_BASE_URL
    : "").trim() || "https://app.aiworklabs.com";

const BUILD_AIWORK_API_URL =
  (typeof import.meta !== "undefined" && typeof import.meta.env?.VITE_DEN_API_BASE_URL === "string"
    ? import.meta.env.VITE_DEN_API_BASE_URL
    : "").trim() || undefined;

export type AiWorkAppBaseUrls = {
  baseUrl: string;
  apiBaseUrl: string;
};

export function normalizeAiWorkAppUrl(input: string | null | undefined): string | null {
...
function isWebAppHost(hostname: string): boolean {
  ...
  return normalized === "app.aiworklabs.com" || normalized === "app.aiwork.software" || normalized.startsWith("app.");
}

function stripApiDenBasePath(input: string | null | undefined): string | null {
  const normalized = normalizeAiWorkAppUrl(input);
...
function deriveApiBaseUrl(input: string | null | undefined): string {
  const normalized = normalizeAiWorkAppUrl(input) ?? DEFAULT_AIWORK_APP_URL;
...
export function resolveAiWorkAppBaseUrls(input: {
  baseUrl?: string | null;
  apiBaseUrl?: string | null;
} | string | null | undefined): AiWorkAppBaseUrls {
  const rawBaseUrl = typeof input === "string" ? input : input?.baseUrl;
  const rawApiBaseUrl = typeof input === "string" ? null : input?.apiBaseUrl;
  const normalizedBaseUrl = normalizeAiWorkAppUrl(rawBaseUrl);
  const normalizedApiBaseUrl = normalizeAiWorkAppUrl(rawApiBaseUrl);
  const seedUrl = normalizedBaseUrl ?? normalizedApiBaseUrl ?? DEFAULT_AIWORK_APP_URL;

  return {
    baseUrl: stripApiDenBasePath(normalizedBaseUrl ?? seedUrl) ?? DEFAULT_AIWORK_APP_URL,
    apiBaseUrl: normalizedApiBaseUrl ?? deriveApiBaseUrl(seedUrl),
  };
}

export { BUILD_AIWORK_API_URL };
