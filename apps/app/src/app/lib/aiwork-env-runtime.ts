const PENDING_CHANGES_KEY = "aiwork.settings.environment.pendingChanges";
...
export function buildAiWorkEnvRuntimeKey(input: {
  baseUrl?: string | null;
  pid?: number | null;
  port?: number | null;
}): string | undefined {
  ...
  return `${baseUrl || "aiwork"}::${runtime || "runtime"}`;
}

export function readAiWorkEnvPendingChanges(runtimeKey?: string | null): boolean {
...
export function writeAiWorkEnvPendingChanges(value: boolean, runtimeKey?: string | null): void {
  const localStorage = getStorage("localStorage");
  const sessionStorage = getStorage("sessionStorage");
  try {
    if (value) {
