...
if (!orchestratorVersion) {
  throw new Error(
    "aiwork-orchestrator version missing in apps/orchestrator/package.json",
  );
}
...
if (!serverVersion) {
  throw new Error(
    "aiwork-server version missing in apps/server/package.json",
  );
}
...
run("pnpm", ["--filter", "aiwork-server", "build:bin:all"], repoRoot);
...
const entries = {
  "aiwork-server": { version: serverVersion, targets: {} },
};

for (const target of targets) {
  const ext = target.id.startsWith("windows") ? ".exe" : "";
  const serverSrc = join(serverDir, `aiwork-server-${target.bun}${ext}`);
  if (!existsSync(serverSrc)) {
    throw new Error(`Missing aiwork-server binary at ${serverSrc}`);
  }
  const serverDest = join(outdir, `aiwork-server-${target.id}${ext}`);
  copyFileSync(serverSrc, serverDest);

  entries["aiwork-server"].targets[target.id] = {
...
writeFileSync(
  join(outdir, "aiwork-orchestrator-sidecars.json"),
