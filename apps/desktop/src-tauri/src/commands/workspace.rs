...
fn collect_workspace_entries(
    ...
    let opencode_dir = workspace_root.join(".aiwork");
...
pub fn workspace_import_config(
    ...
    for i in 0..archive.len() {
        ...
        if !(name == "opencode.json" || name.starts_with(".aiwork/")) {
            continue;
        }
    ...
    let opencode_dir = target_path.join(".aiwork");
    if !opencode_dir.exists() {
        return Err("Archive is missing .aiwork config".to_string());
    }

    let aiwork_path = target_path.join(".aiwork").join("aiwork.json");
