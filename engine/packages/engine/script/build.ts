...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/engine`,
      execArgv: [`--user-agent=engine/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    entrypoints: ["./src/index.ts"],
    define: {
      ENGINE_VERSION: `'${Script.version}'`,
      ENGINE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
    },
  })
