...
import { createEngineClient, type EngineClient, type ToolPart } from "@engine/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run engine with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running engine server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to ENGINE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to ENGINE_SERVER_USERNAME or 'engine')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: EngineClient) {
      ...
      if (args.attach) {
        ...
        const sdk = createEngineClient({ baseUrl: args.attach, directory, headers })
      ...
      const sdk = createEngineClient({ baseUrl: "http://engine.internal", fetch: fetchFn })
      await execute(sdk)
    })
  }),
})
