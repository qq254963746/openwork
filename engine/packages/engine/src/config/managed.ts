...
const MANAGED_PLIST_DOMAIN = "ai.engine.managed"

// Keys injected by macOS/MDM into the managed plist that are not Engine config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/engine"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "engine")
    default:
      return "/etc/engine"
  }
}
...
export function managedConfigDir() {
  return process.env.ENGINE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
