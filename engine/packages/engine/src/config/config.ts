...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for engine serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://www.aiwork.love/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://www.aiwork.love/docs/agents" }),
...
export class Service extends Context.Service<Service, Interface>()("@engine/Config") {}

function globalConfigFile() {
  const candidates = ["engine.jsonc", "engine.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://www.aiwork.love/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "engine.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "engine.jsonc")))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://www.aiwork.love/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ENGINE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/engine` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/engine`))
            if (!response.ok) {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://www.aiwork.love/config.json"
            const source = `${url}/.well-known/engine`
        ...
        if (Flag.ENGINE_CONFIG) {
          yield* merge(Flag.ENGINE_CONFIG, yield* loadFile(Flag.ENGINE_CONFIG))
          log.debug("loaded custom config", { path: Flag.ENGINE_CONFIG })
        }

        if (!Flag.ENGINE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("engine", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.ENGINE_CONFIG_DIR) {
          log.debug("loading config from ENGINE_CONFIG_DIR", { path: Flag.ENGINE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".engine") || dir === Flag.ENGINE_CONFIG_DIR) {
            for (const file of ["engine.json", "engine.jsonc"]) {
          ...
          // Auto-discovered plugins under `.engine/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.ENGINE_CONFIG_CONTENT) {
          const source = "ENGINE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.ENGINE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from ENGINE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ENGINE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ENGINE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.ENGINE_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ENGINE_PERMISSION))
        }
        ...
        if (Flag.ENGINE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ENGINE_DISABLE_PRUNE) {
