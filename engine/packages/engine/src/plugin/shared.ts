...
export const DEPRECATED_PLUGIN_PACKAGES = ["engine-openai-codex-auth", "engine-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, engineVersion: string, pkg?: PluginPackage) {
  ...
  if (!semver.satisfies(engineVersion, range)) {
    throw new Error(`Plugin requires engine ${range} but running ${engineVersion}`)
  }
}
