...
export namespace PluginLoader {
  ...
  > {
    ...
    // npm plugins can declare which engine versions they support; file plugins are treated
    // as local development code and skip this compatibility gate.
    if (base.source === "npm") {
      try {
        await checkPluginCompatibility(base.target, EngineVersion, base.pkg)
      } catch (error) {
  ...
  async function attempt<R>(
    ...
    if (!resolved.ok) {
      if (resolved.stage === "missing") {
        ...
        // for example to load theme files from a plugin package that has no code entrypoint.
        if (missing) {
