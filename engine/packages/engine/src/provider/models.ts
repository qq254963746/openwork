...
import { EngineVersion } from "@/core/env/env.js"
...
export class Service extends Context.Service<Service, Interface>()("@engine/ModelsDev") {}

const USER_AGENT = `engine/${EngineVersion}/${Flag.ENGINE_CLIENT}`
...
export const layer: Layer.Layer<Service, never, AppFileSystem.Service | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.ENGINE_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.ENGINE_MODELS_PATH ?? filepath).pipe(
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.ENGINE_DISABLE_MODELS_FETCH) return {}
      // Flock is cross-process: concurrent engine CLIs can race on this cache file.
    ...
    if (!Flag.ENGINE_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
