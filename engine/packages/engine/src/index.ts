...
import { IS_ENGINE_DEV_MODE, EngineVersion } from "@/core/env/env"
...
const cli = yargs(args)
  ...
  .help("help", "show help")
  .alias("help", "h")
  .version("version", "show version number", EngineVersion)
  .alias("version", "v")
  .option("print-logs", {
  ...
  .middleware(async (opts) => {
    if (opts.pure) {
      process.env.ENGINE_PURE = "1"
    }

    await Log.init({
      print: process.argv.includes("--print-logs"),
      dev: IS_ENGINE_DEV_MODE,
      level: (() => {
        if (opts.logLevel) return opts.logLevel as Log.Level
        if (IS_ENGINE_DEV_MODE) return "DEBUG"
        return "INFO"
      })(),
    })
    ...
    process.env.ENGINE_PID = String(process.pid)

    Log.Default.info("engine", {
