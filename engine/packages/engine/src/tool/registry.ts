...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@/plugin/api"
...
export class Service extends Context.Service<Service, Interface>()("@engine/ToolRegistry") {}
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const questionEnabled =
          ["app", "cli", "desktop"].includes(Flag.ENGINE_CLIENT) || Flag.ENGINE_ENABLE_QUESTION_TOOL
        ...
        return {
          ...
          builtin: [
            ...
            tool.skill,
            tool.patch,
            ...(Flag.ENGINE_EXPERIMENTAL_LSP_TOOL ? [tool.lsp] : []),
            ...(Flag.ENGINE_EXPERIMENTAL_PLAN_MODE && Flag.ENGINE_CLIENT === "cli" ? [tool.plan] : []),
          ],
          task: tool.task,
          read: tool.read,
        }
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const filtered = (yield* all()).filter((tool) => {
        if (tool.id === WebSearchTool.id) {
          return input.providerID === ProviderID.engine || Flag.ENGINE_ENABLE_EXA
        }
