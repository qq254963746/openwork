...
import type { AssistantMessage, Event, EngineClient, SessionMessageResponse, ToolPart } from "@engine/sdk/v2"
...
export function init({ sdk: _sdk }: { sdk: EngineClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `engine auth login` in the terminal",
      name: "Login with engine",
      id: "engine-login",
    }
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const engineProvider = providers.find((p) => p.id === "engine")
  if (engineProvider) {
    const [best] = Provider.sort(Object.values(engineProvider.models))
    if (best) {
      return {
