...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for engine.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal engine sessions
...
engine acp --cwd /path/to/project
```
...
```bash
ENGINE_ENABLE_QUESTION_TOOL=1 engine acp
```
...
- **Terminal Integration**: Full terminal support via engine's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | engine acp
```
...
ACP sessions map cleanly to engine's internal session model:
