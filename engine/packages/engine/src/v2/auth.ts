...
export const OAUTH_DUMMY_KEY = "engine-oauth-dummy-key"
...
export class Service extends Context.Service<Service, Interface>()("@engine/v2/Auth") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const load: () => Effect.Effect<Writable, AuthError> = Effect.fnUntraced(function* () {
      if (process.env.ENGINE_AUTH_CONTENT) {
        try {
          return JSON.parse(process.env.ENGINE_AUTH_CONTENT)
        } catch {}
      }
