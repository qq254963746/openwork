...
- `ENGINE_EXPERIMENTAL_HTTPAPI` selects the backend at server startup. Default is still `hono`.
...
- An Effect `HttpApi` OpenAPI surface exists (`OpenApi.fromApi(PublicApi)` in `cli/cmd/generate.ts --httpapi`, `ENGINE_SDK_OPENAPI=httpapi` in `packages/sdk/js/script/build.ts`) but is opt-in. The default SDK generation is still Hono.
...
- Instance context is provided by `httpapi/server.ts` using `directory`, `workspace`, and `x-engine-directory`.
...
3. Mount the new paths in `src/server/routes/instance/index.ts` only inside the `ENGINE_EXPERIMENTAL_HTTPAPI` block.
...
8. Run `bun typecheck` from `packages/engine`, relevant `bun run test:ci ...` tests from `packages/engine`, and `./packages/sdk/js/script/build.ts` from the repo root.
...
2. The route is mounted by default, not only behind `ENGINE_EXPERIMENTAL_HTTPAPI`.
...
- Cover `directory` and `x-engine-directory` instance selection.
...
14. [ ] Switch OpenAPI/SDK generation to Effect routes and compare SDK output. Effect path is implemented and opt-in via `--httpapi` / `ENGINE_SDK_OPENAPI=httpapi`. Close the schema-shape gaps in `public.ts` (branded `pattern`, per-property `description`, `Event.*` / `SyncEvent.*` naming, dedup collisions), then flip `packages/sdk/js/script/build.ts` default.
15. [ ] Flip `backend.ts` default from `hono` to `effect-httpapi`, keep `ENGINE_EXPERIMENTAL_HTTPAPI` (or its inverse) as a short fallback flag, then delete replaced Hono route files.

## Checklist

- [x] Add first `HttpApi` JSON route slices.
- [x] Bridge selected `HttpApi` routes behind `ENGINE_EXPERIMENTAL_HTTPAPI`. (Now backend-fork-at-startup rather than in-Hono path mounting.)
...
- [x] Implement Effect `HttpApi` OpenAPI generation behind `--httpapi` / `ENGINE_SDK_OPENAPI=httpapi`.
