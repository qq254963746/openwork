...
Practical reference for the current tool-migration state in `packages/engine`.
...
Tool tests should use the existing Effect helpers in `packages/engine/test/lib/effect.ts`:
