...
Practical reference for extracting a future `packages/server` from the current `packages/engine` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all engine services, Effect-first source of truth
- `packages/server` - engine server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full engine server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/engine`.
...
- those slices are mounted into the Hono server behind `ENGINE_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `engine` package split is earlier in the migration.
...
- the current server host still lives in `packages/engine`
...
- if `packages/server` imports services from `packages/engine` while `packages/engine` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/engine` as the temporary Hono host
...
- `packages/server` must not import from `packages/engine`

Allowed in phase 1:

- `packages/engine` imports `packages/server`
...
- `packages/engine` shrinks or disappears as package responsibilities are fully split
...
- `packages/engine` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/engine`
...
Keep in `packages/engine` for now:
...
- host mounting from `packages/engine`
...
- no imports from `packages/engine` inside `packages/server`
...
- `packages/server` must still not import from `packages/engine`
...
### PR 4. Mount `packages/server` question from `packages/engine`

Scope:

- replace local experimental question route wiring in `packages/engine`
...
- mount it in parallel from `packages/engine`
...
- move adapter selection and server startup out of `packages/engine`

Rules:

- do not start this while `packages/server` still depends on `packages/engine`
...
- host logic no longer lives in `packages/engine`
...
3. the route is mounted from the current host in `packages/engine`
...
- `packages/server` imports services or runtime from `packages/engine`
- `packages/engine` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/engine` imports
...
- [ ] mount `question` from `packages/engine`
...
2. keep `packages/engine` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/engine`, stop and restructure the boundary first.
