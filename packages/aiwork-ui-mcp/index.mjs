 ...
 * aiwork-ui-mcp
 *
 * MCP server that exposes AiWork's UI control surface as MCP tools.
 * Speaks MCP stdio and proxies to the AiWork desktop bridge HTTP API.
 *
 * Requires AiWork desktop running with the local UI control bridge active.
 *
 * Usage:
 *   npx aiwork-ui-mcp
 *
...
const DISCOVERY_FILE = "aiwork-ui-control.json";
...
function discoveryPaths() {
  return [
    process.env.AIWORK_UI_CONTROL_DISCOVERY?.trim(),
    join(userAppDataDir(), "com.fengai.aiwork", DISCOVERY_FILE),
    join(userAppDataDir(), "com.fengai.aiwork.dev", DISCOVERY_FILE),
  ].filter(Boolean);
}
...
async function bridgeRequest(path, options = {}) {
  ...
  if (!bridge) {
    return {
      ok: false,
      error: "AiWork is not running. Launch the AiWork desktop app first.",
      hint: "The MCP server connects to a running AiWork instance via its local bridge.",
    };
  }
...
server.tool(
  "ui_snapshot",
  "Get a snapshot of the current AiWork UI state: active route, narration, visible actions, and status. Use this before taking action to understand what the user sees.",
  {},
  async () => {
    ...
    return { content: [{ type: "text", text: lines.join("\n") || "AiWork is reachable, but it did not return visible UI state." }] };
  }
);
...
server.tool(
  "ui_list_actions",
  "List all UI control actions currently available in AiWork: session navigation, composer control, transcript access, and more. Each action has an id you can pass to ui_execute_action.",
  {},
  async () => {
    ...
    if (!Array.isArray(result.actions) || result.actions.length === 0) {
      return { content: [{ type: "text", text: "No actions available. Is AiWork on the main screen?" }] };
    }
...
server.tool(
  "ui_execute_action",
  "Execute an AiWork UI action by its id. Use ui_list_actions first to see available actions and their required arguments.",
  {
...
server.tool(
  "ui_status",
  "Check if AiWork is running and the bridge is reachable. Returns connection status and app info.",
  {},
  async () => {
    const bridge = await discoverBridge();
    if (!bridge) {
      return { content: [{ type: "text", text: "AiWork is not running.\nLaunch the AiWork desktop app to enable UI control." }], isError: true };
    }
    try {
      const response = await fetch(`${bridge.baseUrl}/health`, { signal: AbortSignal.timeout(3000) });
      const data = await response.json();
      return { content: [{ type: "text", text: `Connected to ${data.app || "AiWork"}\nBridge: ${bridge.baseUrl}\nVersion: ${data.version ?? "?"}` }] };
    } catch (error) {
      clearBridgeCache();
      return { content: [{ type: "text", text: `Bridge file found but not reachable: ${error.message}\nAiWork may have quit. Relaunch it.` }], isError: true };
    }
  }
);
