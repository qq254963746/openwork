...
- Hosted web surfaces cover sign-in, worker creation, and related control-plane UX outside this repo.
- The cloud control plane API owns auth/session, worker CRUD, and provisioning orchestration.
- Desktop org runtime config is fetched from the hosted control plane after sign-in and is treated as server-owned runtime policy. It is stored per organization there (`organization.desktop_app_restrictions`) as sparse negative restriction flags (for example `blockZenModel`) and managed from the cloud org settings UI, while install/bootstrap config remains shell-owned in the external bootstrap file and only contains base URL, optional API base URL, and the `forceSignin` startup flag.
...
- Worker preview URLs may be fronted by a provider-aware proxy that refreshes signed links with credentials held server-side.
- The OpenWork app (desktop or mobile client) connects to worker OpenWork server surfaces via URL + token (`/w/ws_*` when available).

```text
Hosted OpenWork Cloud (web + control plane API)
    |
    +--> Worker provisioning / runtime images
    |
    +--> Optional worker preview proxy
