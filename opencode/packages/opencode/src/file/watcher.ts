...
declare const AIWORK_ENGINE_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const binding = require(
      `@parcel/watcher-${process.platform}-${process.arch}${process.platform === "linux" ? `-${AIWORK_ENGINE_LIBC || "glibc"}` : ""}`,
    )
...
export class Service extends Context.Service<Service, Interface>()("@aiwork-engine/FileWatcher") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("FileWatcher.state")(
        function* () {
          if (yield* Flag.AIWORK_ENGINE_EXPERIMENTAL_DISABLE_FILEWATCHER) return
          ...
          if (yield* Flag.AIWORK_ENGINE_EXPERIMENTAL_FILEWATCHER) {
