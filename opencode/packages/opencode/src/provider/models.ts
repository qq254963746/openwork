...
const USER_AGENT = `opencode/${AiWorkEngineVersion}/${Flag.AIWORK_ENGINE_CLIENT}`
...
export const layer: Layer.Layer<Service, never, AppFileSystem.Service | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.AIWORK_ENGINE_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.AIWORK_ENGINE_MODELS_PATH ?? filepath).pipe(
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.AIWORK_ENGINE_DISABLE_MODELS_FETCH) return {}
    ...
    if (!Flag.AIWORK_ENGINE_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
